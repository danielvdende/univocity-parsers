 ...
 */
abstract class SsvFormatDetector implements InputAnalysisProcess {

    private final int MAX_ROW_SAMPLES;
    private final char comment;
    private final String suggestedDelimiter;
    private final char normalizedNewLine;
    private final int whitespaceRangeStart;

    /**
     * Builds a new {@code SsvFormatDetector}
     *
     ...
     */
    SsvFormatDetector(int maxRowSamples, SsvParserSettings settings, int whitespaceRangeStart) {
    ...
    // todo: this probably needs revising to be suitable for string totals calculation.
    ...
    @Override
    // todo; needs revising for string separator.
    public void execute(char[] characters, int length) {
        ...
        String delimiter = min(sums, totals, suggestedDelimiter);
     ...
     */
    private static char min(Map<Character, Integer> map, Map<Character, Integer> totals, String defaultDelimiter) {
        return getChar(map, totals, defaultDelimiter, true);
    }

    /**
     * Returns the string with the highest associated number.
     *
     * @param map               the map of characters and their numbers
     * @param defaultDelimiter  the default string to return in case the map is empty
     *
     * @return the string with the highest number associated.
     */
    private static char max(Map<Character, Integer> map, Map<Character, Integer> totals, String defaultDelimiter) {
        return getChar(map, totals, defaultDelimiter, false);
    }
     ...
     */
    // TODO: refactor this to make sense for string separator.
    private static char getChar(Map<Character, Integer> map, Map<Character, Integer> totals, String defaultDelimiter, boolean min) {
    ...
    /**
     * Applies the discovered CSV format elements to the {@link SsvParser}
     *
     * @param delimiter   the discovered delimiter string
     * @param quote       the discovered quote character
     * @param quoteEscape the discovered quote escape character.
     */
    abstract void apply(String delimiter, char quote, char quoteEscape);
}
